            ...
            if self.state.postgres_port in ports_used:
                self.errors.append(
                    ValidationError(
                        field="postgres_port",
                        message=(
                            f"Port {self.state.postgres_port} conflicts with "
                            f"{ports_used[self.state.postgres_port]} service"
                        ),
                    )
                )
                return False
            ...
            if self.state.temporal_ui_port in ports_used:
                self.errors.append(
                    ValidationError(
                        ...
                        message=(
                            f"Port {self.state.temporal_ui_port} conflicts with "
                            f"{ports_used[self.state.temporal_ui_port]} service"
                        ),
                    )
                )
                return False
            ...
            if self.state.temporal_frontend_port in ports_used:
                self.errors.append(
                    ValidationError(
                        ...
                        message=(
                            f"Port {self.state.temporal_frontend_port} conflicts with "
                            f"{ports_used[self.state.temporal_frontend_port]} service"
                        ),
                    )
                )
                return False
