...
class AgentCache:
    ...
    def clear(self) -> None:
        ...
        self._stats = {"hits": 0, "misses": 0, "evictions": evictions, "puts": 0}
...
class AgentDiscovery:
    ...
    def __init__(self, index_path: Path, cache_size: int = 50):
        ...
        self._stats = {"file_reads": 0, "total_lookups": 0}
    ...
    def _load_index(self) -> dict[str, Any]:
        ...
        try:
            ...
            raise MalformedIndexError(f"Invalid JSON in index file: {e}") from e

        if "agents" not in data:
            raise MalformedIndexError("Index missing required 'agents' key")

        if not isinstance(data["agents"], list):
            raise MalformedIndexError("'agents' must be a list")
    ...
    def list_agents(self, category: str | None = None, keywords: list[str] | None = None) -> list[dict[str, Any]]:
        ...
        agents = self._category_to_agents.get(category, []) if category else self.index["agents"]
    ...
    def search(self, query: str) -> list[dict[str, Any]]:
        ...
        results = [self._id_to_agent[agent_id] for agent_id in results_set if agent_id in self._id_to_agent]
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "cache_hit_rate": (cache_stats["hits"] / max(1, self._stats["total_lookups"])) * 100,
        }
...
def benchmark_discovery(index_path: Path) -> dict[str, float]:
    ...
    categories = {a.get("category") for a in agents if "category" in a}
    if categories:
